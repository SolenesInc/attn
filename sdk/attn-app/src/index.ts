...
// React, re-exported by name: `export *` would make React's whole surface the SDK's contract.
export {
  Fragment,
  useCallback,
  useEffect,
...
/** One fact from attn's durable event bus. A fact is an invalidation, not a payload. */
export interface AppEvent {
  ...
  readonly seq: number
  /** The producer's payload, when it carries one. Unknown until a fact class is pinned. */
...
export interface Document<Body = unknown> {
  readonly id: string
  readonly body: Body
  /** Counts writes to this document. Hand it back to put() to refuse an edit built on a replaced version. */
  readonly rev: number
...
export interface PutOptions {
  /** The rev the caller read. The write is refused when the stored document has moved on. */
  ifRev?: number
}

/** One of the app's document collections. The namespace is derived from the app's name, never passed in. */
...
export interface AppContext<Collections> {
  ...
  /** Read-only current truth, the same projection attn's own UI is handed. */
  readonly current: {
    /** One consistent read of attn's state-bearing domains, stamped with the bus position it was taken at. */
    snapshot(): Promise<CurrentStateSnapshot>
  }
}

/** A handler. Throwing fails that delivery: the bus retries with backoff rather than skipping. */
...
/** Rebuilds the app's collections from `ctx.current.snapshot()`. It must converge, delete as
    well as upsert, and yield; while a rebuild is owed, commands are refused with `reconcile_owed`. */
...
// Styles come from attn's own build, not from here — see components.tsx.
