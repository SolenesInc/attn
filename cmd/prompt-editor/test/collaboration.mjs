...
try {
    ...
    await expect.poll(() => page.evaluate(() => navigator.clipboard.readText())).toContain(`context --review ${reviewID} --json`);
    const handoff = await page.evaluate(() => navigator.clipboard.readText());
    assert.ok(handoff.includes("Before editing:") && handoff.includes("reread the complete affected prompts"));
    const agentContext = cli("context", "--review", reviewID);
    assert.equal(agentContext.sources[source].current.text, "Keep this concurrent agent edit.\n");
    assert.ok(agentContext.scenarios.some((item) => item.current.text === "Keep this concurrent agent edit."));
    if (artifacts)
