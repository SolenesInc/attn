...
export function clearDelegationModelCatalogs() { catalogs.clear(); inflight.clear(); }
...
function useCatalog(harness: DelegationHarness | undefined, loadModels: (harness: string) => Promise<DelegationModelCatalog>) {
  ...
  const discover = (force = false) => {
    if (!id || (!force && (catalogs.has(id) || inflight.has(id)))) return;
    if (force) catalogs.delete(id);
    setErrors(current => ({ ...current, [id]: '' }));
    const request = loadModels(id).then(result => { catalogs.set(id, result); return result; })
      .catch((e: unknown) => { setErrors(current => ({ ...current, [id]: e instanceof Error ? e.message : String(e) })); return { models: [], detail: '' }; })
      .finally(() => { inflight.delete(id); rerender(n => n + 1); });
    inflight.set(id, request);
    rerender(n => n + 1);
  };
  useEffect(() => { if (harness?.discovery) discover(); }, [id]); // eslint-disable-line react-hooks/exhaustive-deps
  return { catalog: catalogs.get(id), loading: inflight.has(id), error: errors[id] ?? '', discover };
}
