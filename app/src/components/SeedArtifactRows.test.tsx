...
describe('SeedArtifactRows', () => {
  ...
  it('opens, reveals, and moves a managed file out through typed actions', async () => {
    const daemon = api();
    vi.mocked(save).mockResolvedValue('/tmp/out/report.bin');
    render(
      <DaemonApiProvider api={daemon}>
        <SeedArtifactRows
          seedId="s-1"
          artifacts={[{ filename: 'report.bin', relative_target: 'report.bin', size: 12, modified_at: '2026-08-29T20:00:00Z' }]}
        />
      </DaemonApiProvider>,
    );

    expect(screen.getByText('report.bin').closest('li')).toHaveTextContent('12 bytes');
    fireEvent.click(screen.getByRole('button', { name: 'Reveal' }));
    await waitFor(() => expect(invoke).toHaveBeenCalledWith('open_safe_seed_artifact_target', {
      path: '/notebook/seeds/s-1/report.bin', reveal: true,
    }));
    ...
    await waitFor(() => expect(daemon.sendSeedArtifactTransfer).toHaveBeenCalledWith({
      seedId: 's-1', operation: 'detach', filename: 'report.bin', destinationPath: '/tmp/out/report.bin',
    }));
  });
