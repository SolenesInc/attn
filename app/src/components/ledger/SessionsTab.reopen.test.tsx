...
async function refuseFirstReopen(view: { settle: () => Promise<void> }, label: string) {
...
describe('SessionsTab reopens on demand', () => {
  it('offers Reopen on every closed row and Focus on a live one without asking for eligibility', async () => {
    const view = await renderSessionsTab({ answer: pages([page({ entries: [closedEntry('s1'), closedEntry('s2'), liveEntry('s3')] })]), onReopen: vi.fn() });
  ...
  it('offers nothing without a hand to run it', async () => {
    await renderSessionsTab({ answer: pages([page({ entries: [closedEntry('s1')] })]) });
  ...
  it('shows the row busy while the daemon works and clears it on success', async () => {
    ...
    const view = await renderSessionsTab({ answer: pages([page({ entries: [closedEntry('s1')] })]), onReopen });
  ...
  it('turns a refusal into the actions the session offers instead, and reads them in the inspector', async () => {
    ...
    const view = await renderSessionsTab({ answer: pages([page({ entries: [closedEntry('s1', { branch: 'feat/x' })] })]), onReopen, workspaceNames: { 'ws-1': 'attn' } });
  ...
  it('replaces a live row when the daemon says it closed, and offers Reopen without re-listing', async () => {
    const view = await renderSessionsTab({ answer: pages([page({ entries: [liveEntry('s1')] })]), onReopen: vi.fn() });
  ...
  it('hides stale pagination while replacing the current page', async () => {
    const view = await renderSessionsTab({
      answer: (_query, index) => (index === 0 ? page({ entries: [closedEntry('s1')], next_before: 'older', omitted: 1 }) : 'hold'),
    });
    expect(rows().getByText('run s1')).toBeInTheDocument();
...
describe('SessionsTab row grammar', () => {
  it('names the session that closed another, falls back to its id, and says you for the user', async () => {
    await renderSessionsTab({ answer: pages([page({ entries: [
  ...
  it('shows a worktree verb until a refusal says the directory is gone, and copies the path with y', async () => {
    ...
    const view = await renderSessionsTab({
      answer: pages([page({ entries: [closedEntry('here', { is_worktree: true }), closedEntry('gone', { is_worktree: true })] })]),
      onReopen,
      onShowWorktree,
    });
