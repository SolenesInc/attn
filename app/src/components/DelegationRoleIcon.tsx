...
export function DelegationRoleIcon({ icon, name }: { icon: string; name: string }) {
  if (!ICON_PATHS[icon]) return <span aria-hidden="true">{name.trim().slice(0, 1).toUpperCase() || '○'}</span>;
  return <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{ICON_PATHS[icon]}</svg>;
}
