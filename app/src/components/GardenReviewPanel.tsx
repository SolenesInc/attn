...
function composerPrompt(kind: ComposerKind): string {
  if (kind === 'handover') return 'What should the new agent know?';
  if (kind === 'chief') return 'What should Chief know?';
  if (kind === 'park') return 'Why are you parking this?';
  return 'Why should this be withered?';
}

function composerProgress(kind: ComposerKind): string {
  if (kind === 'handover') return 'Handing over…';
  if (kind === 'chief') return 'Sending to Chief…';
  return `${actionLabels[kind]}ing…`;
}

function ReviewComposer({ item, composer, onChange, onCancel, onDraft, onSubmit }: {
  item: GardenReviewItem;
  ...
  const canDraft = composer.kind === 'handover' && !composer.busy && !composer.drafting;
  return <form className="garden-review__composer" onSubmit={(event) => { event.preventDefault(); if (!composer.busy) onSubmit(); }}>
    <div className="garden-review__composer-head">
      <label htmlFor={`garden-review-compose-${item.id}`}>{composerPrompt(composer.kind)}<span>optional</span></label>
      {canDraft && <button type="button" className="garden-review__draft" onClick={onDraft}>Draft</button>}
      {composer.drafting && <span aria-live="polite">Drafting…</span>}
    </div>
    <textarea
      id={`garden-review-compose-${item.id}`}
      ref={textareaRef}
      value={composer.text}
      onChange={(event) => onChange({ ...composer, text: event.target.value, pendingDraft: '' })}
      onKeyDown={(event) => {
    ...
    />
    {composer.kind === 'handover' && <DelegationCheckoutFields value={composer} onChange={(patch) => onChange({ ...composer, ...patch })} />}
    {composer.pendingDraft && <p className="garden-review__draft-ready">Draft ready. Your edits were kept.
      <button type="button" onClick={() => onChange({ ...composer, text: composer.pendingDraft, pendingDraft: '' })}>Use draft</button>
    </p>}
    {composer.error && <p className="garden-review__error" role="alert">{composer.error}</p>}
    <div className="garden-review__composer-actions">
      {composer.busy ? <span aria-live="polite">{composerProgress(composer.kind)}</span> : <>
...
function createReviewActions({ review, composer, fetchSeedDocument, onMoveSeed, onResumeSeed, onKeepSeed, onHandoverSeed, onSendSeedToChief, onRetry, onDraft, onRefresh, setComposer, setActionError, setRetrying }: {
  review: GardenReview;
  composer: ComposerState | null;
  fetchSeedDocument: GardenReviewPanelProps['fetchSeedDocument'];
  onMoveSeed: GardenReviewPanelProps['onMoveSeed'];
  onResumeSeed: GardenReviewPanelProps['onResumeSeed'];
  onKeepSeed: GardenReviewPanelProps['onKeepSeed'];
  onHandoverSeed: GardenReviewPanelProps['onHandoverSeed'];
  onSendSeedToChief: GardenReviewPanelProps['onSendSeedToChief'];
  onRetry: GardenReviewPanelProps['onRetry'];
  onDraft: GardenReviewPanelProps['onDraft'];
  onRefresh: GardenReviewPanelProps['onRefresh'];
  setComposer: React.Dispatch<React.SetStateAction<ComposerState | null>>;
  setActionError: React.Dispatch<React.SetStateAction<string>>;
  setRetrying: React.Dispatch<React.SetStateAction<boolean>>;
}) {
  const refresh = () => onRefresh(review.run.id);
  const runResume = async (item: GardenReviewItem) => {
    setActionError('');
    try {
  ...
  const runKeep = async (item: GardenReviewItem) => {
    setActionError('');
    try {
  ...
  const runHarvest = async (item: GardenReviewItem) => {
    setActionError('');
    try {
      const reason = lifecycleReason(advisorGuidance(item, 'harvest')) || REVIEW_HARVEST_REASON;
      await onMoveSeed(item.seed_id, 'harvest', reason, undefined, undefined, reviewContext(review, item));
      await refresh();
    } catch (error) {
  ...
  const submitLifecycle = async (item: GardenReviewItem, state: ComposerState) => {
    const text = state.text.trim();
    setComposer({ ...state, busy: true, error: '' });
    try {
      await onMoveSeed(item.seed_id, state.kind, state.kind === 'park' ? undefined : text || undefined, undefined, state.kind === 'park' ? text || undefined : undefined, reviewContext(review, item));
      setComposer(null);
      await refresh();
    } catch (error) {
      setComposer((current) => current && current.itemId === item.id ? { ...current, busy: false, error: error instanceof Error ? error.message : `${actionLabels[state.kind]} failed` } : current);
      await refresh().catch(() => {});
    }
  };
  const submitHandover = async (item: GardenReviewItem, state: ComposerState) => {
    setComposer({ ...state, busy: true, error: '' });
    try {
  ...
  const submitChief = async (item: GardenReviewItem, state: ComposerState) => {
    ...
    try {
      ...
      setComposer((current) => current && current.itemId === item.id ? { ...current, busy: false, error: error instanceof Error ? error.message : 'Sending the seed to Chief failed' } : current);
      await refresh().catch(() => {});
    }
  };
  const requestDraft = async (item: GardenReviewItem) => {
    ...
    try {
      ...
      setComposer((current) => current && current.itemId === item.id ? { ...current, drafting: false, error: error instanceof Error ? error.message : 'Drafting the handoff failed' } : current);
    }
  };
  const beginHandover = async (item: GardenReviewItem) => {
    setActionError('');
    try {
      setComposer(emptyComposer(item, 'handover', await fetchSeedDocument(item.seed_id)));
    } catch (error) {
      setActionError(error instanceof Error ? error.message : 'Could not read the saved working context');
    }
  };
  const retry = async (item: GardenReviewItem) => {
    setRetrying(true);
    setActionError('');
    try {
  ...
  return { runResume, runKeep, runHarvest, submitLifecycle, submitHandover, submitChief, requestDraft, beginHandover, retry };
}

type ReviewActions = ReturnType<typeof createReviewActions>;

function ReviewAdvice({ selected }: { selected: GardenReviewItem }) {
  if (!selected.recommendation) return null;
  return <section className="garden-review__advice">
    ...
    {selected.cited_evidence && selected.cited_evidence.length > 0 && <ul>{selected.cited_evidence.map((entry) => <li key={entry}>{entry}</li>)}</ul>}
  </section>;
}

function ReviewProblem({ selected, retrying, onRetry }: { selected: GardenReviewItem; retrying: boolean; onRetry: () => void }) {
  const failed = selected.status === 'failed';
  return <section className="garden-review__problem">
    <h3>{failed ? 'The advisor could not review this seed' : 'This seed changed'}</h3>
    <p>{failed ? selected.advisor_error || advisorFailureWords(selected.error) : selected.error || 'Refresh the evidence and try again.'}</p>
    {retrying ? <span aria-live="polite">Refreshing…</span> : <button type="button" onClick={onRetry}>Try again</button>}
  </section>;
}

function ReviewStatusPanel({ review, selected, atReviewSeed, retrying, onReturn, onRetry }: {
  review: GardenReview;
  selected: GardenReviewItem;
  atReviewSeed: boolean;
  retrying: boolean;
  onReturn: () => void;
  onRetry: () => void;
}) {
  if (!atReviewSeed) return <section className="garden-review__context">
    ...
    <button type="button" onClick={onReturn}>Return to review seed</button>
  </section>;
  if (selected.status === 'ready' && selected.resolution === 'unresolved') return <ReviewAdvice selected={selected} />;
  if (selected.status === 'failed' || selected.status === 'invalidated') return <ReviewProblem selected={selected} retrying={retrying} onRetry={onRetry} />;
  return <AdvisorProgress review={review} item={selected} />;
}

function ReviewActionPanel({ selected, visibleActions, composer, actions, onSendSeedToChief, setComposer }: {
  selected: GardenReviewItem;
  ...
  const choose = (action: string) => {
    if (action === 'resume') void actions.runResume(selected);
    else if (action === 'keep_growing') void actions.runKeep(selected);
    else if (action === 'harvest') void actions.runHarvest(selected);
    else if (action === 'send_to_chief' && onSendSeedToChief) setComposer(emptyComposer(selected, 'chief'));
    else if (action === 'handover') void actions.beginHandover(selected);
    else if (action === 'park' || action === 'wither') setComposer(emptyComposer(selected, action));
  };
  return <section className="garden-review__actions">
    <h3>What should happen?</h3>
    {composer?.itemId === selected.id ? <ReviewComposer
      item={selected}
      composer={composer}
      onChange={setComposer}
      onCancel={() => setComposer(null)}
      onDraft={() => void actions.requestDraft(selected)}
      onSubmit={() => {
        if (composer.kind === 'handover') void actions.submitHandover(selected, composer);
        else if (composer.kind === 'chief') void actions.submitChief(selected, composer);
        else void actions.submitLifecycle(selected, composer);
      }}
    /> : <div className="garden-review__action-row" aria-label="Choose what happens to this seed">
      {visibleActions.map((action) => <button type="button" key={action} className={action === selected.recommendation ? 'is-recommended' : ''} onClick={() => choose(action)}>{actionLabels[action] ?? action}</button>)}
    </div>}
  </section>;
}

function ReviewDecision({ review, selected, atReviewSeed, retrying, ordinaryEvidence, actionError, actionable, visibleActions, composer, actions, onSendSeedToChief, setComposer, onReturn }: {
  review: GardenReview;
  selected: GardenReviewItem;
  atReviewSeed: boolean;
  retrying: boolean;
  ordinaryEvidence: GardenReviewItem['evidence'];
  actionError: string;
  actionable: boolean;
  visibleActions: string[];
  composer: ComposerState | null;
  ...
  return <aside className="garden-review__decision">
    <ReviewStatusPanel review={review} selected={selected} atReviewSeed={atReviewSeed} retrying={retrying} onReturn={onReturn} onRetry={() => void actions.retry(selected)} />
    {atReviewSeed && <section className="garden-review__evidence"><h3>What we know</h3><dl>{ordinaryEvidence.map((entry) => <div key={entry.label}><dt>{entry.label}</dt><dd>{entry.text}</dd></div>)}</dl></section>}
    {actionError && <p className="garden-review__error" role="alert">{actionError}</p>}
    {atReviewSeed && actionable && <ReviewActionPanel selected={selected} visibleActions={visibleActions} composer={composer} actions={actions} onSendSeedToChief={onSendSeedToChief} setComposer={setComposer} />}
  </aside>;
}

function ReviewHeader({ frame, complete, unresolved, advised, total, onExit, onClose, onToggleFrame }: {
  frame: 'dock' | 'full'; complete: boolean; unresolved: number; advised: number; total: number;
  onExit: () => void; onClose: () => void; onToggleFrame?: () => void;
}) {
  return <header className="garden-review__chrome">
    <button type="button" className="garden-review__back" onClick={onExit}>‹ Garden</button>
    <div className="garden-review__title"><strong>Review garden</strong><span>{complete ? 'complete' : `${unresolved} left · ${advised}/${total} advised`}</span></div>
    {onToggleFrame && <button type="button" className="garden-review__frame" onClick={onToggleFrame} aria-label={frame === 'full' ? 'Return the garden to the dock' : 'Expand the garden'}><FrameGlyph direction={frame === 'full' ? 'in' : 'out'} /></button>}
    <button type="button" className="garden-review__close" onClick={onClose} aria-label="Close">×</button>
  </header>;
}

function ReviewRail({ items, selectedId, onSelect }: { items: GardenReviewItem[]; selectedId?: string; onSelect: (id: string) => void }) {
  return <nav className="garden-review__rail" aria-label="Seeds in this review">{items.map((item, index) => <button type="button" key={item.id} className={`${item.id === selectedId ? 'is-current' : ''}${item.resolution !== 'unresolved' ? ' is-resolved' : ''}`} onClick={() => onSelect(item.id)} aria-current={item.id === selectedId ? 'step' : undefined}>
    <span className="garden-review__rail-number">{index + 1}</span><span className="garden-review__rail-copy"><strong>{item.title}</strong><span>{statusWords(item)}</span></span>
  </button>)}</nav>;
}
  ...
  return <main className="garden-review__reader">
    <article className="garden-review__seed">
      <nav className="garden-review__seed-nav" aria-label="Plot path">
        {!atReviewSeed && <button type="button" className="garden-review__return" onClick={returnToReviewSeed}>‹ Review seed</button>}
        {path.length > 0 && <ol>{path.map((seed, index) => <li key={seed.id}>{index < path.length - 1 ? <button type="button" onClick={() => openSeed(seed.id)}>{seed.title}</button> : <span aria-current="page">{seed.title}</span>}</li>)}</ol>}
      </nav>
      <div className="garden-review__seed-head"><div><span className="garden-review__eyebrow">{browsedSeed?.id ?? browsedSeedId}</span><h2>{browsedSeed?.title ?? selected.title}</h2></div><span className={`garden-review__status is-${atReviewSeed ? selected.status : browsedSeed?.status ?? 'queued'}`}>{atReviewSeed ? statusWords(selected) : browsedSeed?.status}</span></div>
      ...
      {browsedDocument && <SeedDocumentView key={browsedDocument.seed.id} document={browsedDocument} compact ledgerInitiallyOpen onOpenSeed={openSeed} />}
    </article>
    {decision}
  </main>;
}
...
export function GardenReviewPanel({
  ...
  const actions = createReviewActions({
    review, composer, fetchSeedDocument, onMoveSeed, onResumeSeed, onKeepSeed, onHandoverSeed,
    onSendSeedToChief, onRetry, onDraft, onRefresh, setComposer, setActionError, setRetrying,
  });
  ...
  const decision = selected ? <ReviewDecision review={review} selected={selected} atReviewSeed={atReviewSeed} retrying={retrying} ordinaryEvidence={ordinaryEvidence} actionError={actionError} actionable={actionable} visibleActions={visibleActions} composer={composer} actions={actions} onSendSeedToChief={onSendSeedToChief} setComposer={setComposer} onReturn={returnToReviewSeed} /> : null;
