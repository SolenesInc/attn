...
export function useAgentNavigation(options: {
  ...
  const pendingSelectionRef = useRef<{ sessionId: string; seen: boolean } | null>(null);
  const cancelPendingSelection = useCallback(() => {
    pendingSelectionRef.current = null;
  }, []);

  const focusAgentPane = useCallback(
    (sessionId: string, paneId: string, activateSession: boolean) => {
    ...
    [
      options.focusSessionPane,
      options.revealSessionView,
      options.requestTerminalFocus,
      options.setActivePane,
      options.setActiveSession,
    ],
  );
  ...
  const selectAgent = useCallback(
    (sessionId: string) => {
      ...
      if (!pane) {
        pendingSelectionRef.current = { sessionId, seen: Boolean(session) };
        return false;
      }
  ...
  useEffect(() => {
    ...
    const exists = options.sessions.some((session) => session.id === sessionId);
    if (!exists) {
      if (pending.seen) pendingSelectionRef.current = null;
      return;
    }
    selectAgent(sessionId);
  }, [options.sessions, selectAgent]);

  const selectAgentPane = useCallback(
    (sessionId: string, paneId: string) => {
      pendingSelectionRef.current = null;
      const session = options.sessions.find((entry) => entry.id === sessionId);
      const pane = session?.workspace.agents.find(
        (entry) => entry.id === paneId && entry.sessionId === sessionId,
      );
      if (!pane) {
        return false;
      }
  ...
  const move = useCallback(
    (direction: AgentHistoryDirection, resumeCurrent = false) => {
      pendingSelectionRef.current = null;
