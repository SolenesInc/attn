...
// Two conversation panes are mounted at once later, so every selector is
// scoped to the session it belongs to.
...
// The host executable by its own path, not the substring `attn-nisse`: a
// profile named `nisse*` puts that in every sibling process's bundle path.
...
// A pid the kernel has since handed to something else is not our process,
// so pid AND command have to match.
...
async function main() {
  ...
  try {
    ...
    await runner.step('reload_resumes_with_history', async () => {
      ...
      // The AGENT has the history, not just the pane: only a reopened session
      // file can answer this.
    ...
    await runner.step('a_cold_client_draws_the_whole_conversation', async () => {
      // The new window never saw this host's stream, so what it draws came from
      // the snapshot it asked for on mount.
    ...
    await runner.step('killed_before_its_first_word_relaunches_clean', async () => {
      ...
      // killed this early leaves nothing to reopen.
      const before = hostProcesses().map((entry) => entry.pid);
      const created = await client.request('create_session', {
        cwd: repoDir,
