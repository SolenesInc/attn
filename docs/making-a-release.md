...
Neither `make test` nor CI runs the release tooling's tests. Run
`make test-scripts` after changing a release script, a gate, or
`cmd/release-train`, and before preparing a candidate.
