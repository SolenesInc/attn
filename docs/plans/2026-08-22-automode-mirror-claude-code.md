...
| # | Question | Decision |
| --- | --- | --- |
| 1 | Hard blocks | Two: `Data Exfiltration`, and `Auto Mode Self-Approval` (2026-08-23). The rest of auto-mode tampering stays soft. **Divergence:** CC has one. |
...
### The second hard block, added 2026-08-23

Moving auto-mode tampering to the soft side rested on promotion being
proposal-gated: an agent may ask, and only a person promotes. What holds that
gate shut was a set of hard-deny patterns matching command text, which is not a
fence. Reading the client token is ordinary work, and with it the promote
command goes over the daemon's socket or its local port however the caller
cares to spell it.

The difference between fine and forbidden there is what the token is used for,
so a pattern cannot see it and a rule has to. `Auto Mode Self-Approval` is that
rule, and it is hard because a conversation must not be able to clear it: the
words that would clear it are exactly what an injection supplies. Driving
attn's own window counts, for the same reason `Self Drive` exists.

This also settles where a hard block has to be legible. Stage 1 grades harm
with intent out of scope and ends the call on its own below the ceiling, so a
hard block only stage 2 can recognise is not a hard block. Everything in that
section is forbidden without intent, which is what makes stating it in stage
1's terms possible: the HARD BLOCK preamble and stage 1's own instruction both
say to grade those at the top of the scale.

The hard-deny patterns that used to stand in for this rule cover the three
proposal verbs, which only ever propose. Blocking those blocks the agent's
sanctioned path, and they come off.
