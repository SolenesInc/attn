...
- Repository rules: version-controlled prefix rules in a checkout's `.attn/rules.json`, merged with daemon-owned and shipped rules when a session launches. Strictest matching behavior wins. Checkout rules may skip review inside the sandbox or request a reviewed sandbox escape, but cannot grant unreviewed sandbox bypasses.
