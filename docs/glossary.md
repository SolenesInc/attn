...
- Delegation popup lifecycle: a provider-local controller owns trigger identity, dismissal and focus return across sidebar rows, header badges and the Action menu. Trigger wiring reports pointer events; the popup renders the chain and releases its focus trap before selecting an agent. Positioning owns geometry only. App reports navigation, blocking surfaces and sidebar collapse rather than keeping a second popup focus target.
