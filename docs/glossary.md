...
b`, recording that a fell out of work on b). A seed may name several origins;
...
not parked, blocked, held, a gate, a packet, or under a packet. A plot is never
ready itself; its children are. Readiness is computed when asked and never stored, so harvesting
a blocker frees its dependent at the next call, with nobody clearing anything.
`attn seed ready` answers for the whole garden unless the session reports to a
seed, when its flag-free answer comes from that seed's plot. `--all` steps back
out to the whole garden. `attn seed prime` carries the same current answer in
the guidance injected at session start and after compaction. The garden is one
...
`ready` answers with the plot, and `attn seed prime` points at the plot seed for
