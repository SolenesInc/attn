...
- Member home: canonical charter/handoff directory, resolved and served only by the home daemon. Charter replacements require the token for the exact bytes read; every accepted replacement advances that token, including identical content, so a confirmation fences older delayed writes. Handoff history returns each complete filed letter. Registry: index of member files. Binding: member's active session. Launch settings: optional member harness, model, and effort pins; blanks resolve through daemon and harness defaults. A one-day wake harness override ends with that day.
