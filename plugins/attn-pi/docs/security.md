...
own logging. Bare pi's `/security network allow` is unrestricted, on both
platforms; `/security network deny` blocks tool networking outright, including
localhost. Under attn, a bash command's network instead goes through attn's
proxy, which enforces the host allow and deny lists, on macOS. On Linux the
sandbox has no network at all under attn, until seed s-00asxp brings the proxy
into its own namespace. Either way, if attn's proxy is not actually running
when network policy is on, the sandbox has no network on either platform, and
the session is told so once at start; a command that needs the network asks
for `sandbox_permissions=require_escalated` and runs outside the sandbox after
review. Reachable filesystem sockets, such as an SSH agent or Docker socket,
remain trusted services with their own authority.
