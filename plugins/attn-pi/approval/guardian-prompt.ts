...
/** maxActionBytes caps the whole action JSON; maxActionStringTokens caps any one string inside it. */
...
// pi has no developer role, so this reminder opens the first delta review instead of riding a separate message.
...
/** Thrown when the action JSON won't fit; the caller turns this into a denial that names the limit. */
