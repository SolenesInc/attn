// Every entry here runs UNJUDGED. So a command that can be talked into writing carries the
// flags that keep it read-only, and one that can run another command is not in the set.
...
// Scans the whole command line, not only the leading word, so a network call cannot ride
// in as a later element of something that otherwise parses as read-only.
