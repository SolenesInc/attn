...
// `--dev` gives a private /dev/shm; a writable root rebinds the host's.
export function bwrapArgs(spec: SandboxSpec, command: string[]): string[] {
  ...
  // A shared namespace would let a proxied command bypass the host allowlist.
  // The proxy bridge into this namespace is seed s-00asxp.
  if (spec.network.mode !== "unrestricted") args.push("--unshare-net");
