...
function networkPolicy(spec: SandboxSpec): string[] {
  switch (spec.network.mode) {
    ...
    case "proxy": return [`(allow network-outbound (remote ip "localhost:${spec.network.proxy.port}"))`, seatbeltNetwork];
    case "unrestricted": return ["(allow network*)", seatbeltNetwork];
  }
}
