export function normalizeHost(input: string): string {
  ...
  // Strip :port only when there is exactly one colon, so unbracketed IPv6 literals survive.
...
// Reachable only when the target host named that exact address, or is localhost and the address is loopback.
...
// example.com is exact, *.example.com is subdomains only, **.example.com is apex+subdomains, * is everything.
