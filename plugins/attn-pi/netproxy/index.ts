...
// 2s timeout (Codex runtime.rs:50); a timeout denies the host like a failed lookup.
const dnsLookupTimeoutMs = 2_000;

// 200-entry cap (Codex runtime.rs:49) scoped per credentials, not global.
...
export class NetworkProxy implements ProxyGate {
  ...
  // Bucketed under "" when credentials aren't ours, so a spoofed value can't grow the map unbounded.
  ...
  // A failed lookup also returns true (fail closed); dial() re-resolves before connecting.
