...
// 4x Node's 16 KiB header default; a tripwire, never hit by a real request head.
...
export async function serveHttp(client: Socket, reader: SocketReader, gate: ProxyGate): Promise<void> {
  ...
  // A mismatched Host header would let the upstream serve a different request than we authorized.
...
function parseHead(raw: Buffer): RequestHead | undefined {
  ...
  // A bare LF or CR here is a smuggled second request; refuse rather than forward it.
...
// Headers are rebuilt fresh, dropping hop-by-hop ones and forcing Connection: close, so
// a second request can't ride this tunnel to an unchecked host.
