...
// "No authentication" is refused: every connection must carry one session's proxy credentials.
...
async function readAddress(reader: SocketReader, type: number): Promise<string | undefined> {
  ...
  // Canonicalize so a deny rule written in short form still matches this expanded address.
