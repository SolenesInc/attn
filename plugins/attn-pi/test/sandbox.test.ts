...
import { seatbeltInvocation } from "../sandbox/seatbelt";
...
function config(overrides: Partial<SandboxConfig> = {}): SandboxConfig {
  return { mode: "workspace-write", network: "off", allowWrite: [], denyRead: [], denyWrite: [], cacheWritePaths: [], ...overrides };
}
...
describe("sandboxSpecFor", () => {
  ...
  test("network mode carries the proxy when one is running, and unrestricted ignores it", () => {
    const { cwd, temp, cleanup } = workspace();
    const proxy: ProxyAddress = { host: "127.0.0.1", port: 4711, credentials: "run-token" };
    try {
      expect((sandboxSpecFor(config({ network: "proxy" }), cwd, temp, { permissions: "use_default", proxy }) as SandboxSpec).network)
        .toEqual({ mode: "proxy", proxy });
      expect((sandboxSpecFor(config({ network: "unrestricted" }), cwd, temp, { permissions: "use_default", proxy }) as SandboxSpec).network)
        .toEqual({ mode: "unrestricted" });
      expect((sandboxSpecFor(config({ network: "off" }), cwd, temp, { permissions: "use_default", proxy }) as SandboxSpec).network)
        .toEqual({ mode: "off" });
    } finally { cleanup(); }
  });

  test("a proxy policy with no running proxy fails closed instead of running unrestricted", () => {
    const { cwd, temp, cleanup } = workspace();
    try {
      expect((sandboxSpecFor(config({ network: "proxy" }), cwd, temp, { permissions: "use_default" }) as SandboxSpec).network)
        .toEqual({ mode: "off" });
    } finally { cleanup(); }
  });
});
...
describe.skipIf(process.platform !== "darwin")("seatbelt enforcement", () => {
  ...
  test("a proxied profile reaches the proxy port and nothing else", async () => {
    ...
    try {
      const spec = sandboxSpecFor(config({ network: "proxy" }), cwd, temp, {
  ...
  test("a proxy policy with no running proxy reaches nothing, same as offline", async () => {
    const { cwd, temp, cleanup } = workspace();
    const target = await listener();
    try {
      const spec = sandboxSpecFor(config({ network: "proxy" }), cwd, temp, { permissions: "use_default" });
...
describe("bwrap arguments", () => {
  ...
  test("the workspace is bound writable, the protected path read-only, and the network namespace is private unless the network is unrestricted", () => {
    const { cwd, temp, secret, cleanup } = workspace();
    const guarded = join(cwd, ".pi");
    mkdirSync(guarded);
    try {
      ...
      // Bare pi, no attn proxy: matches Seatbelt's unrestricted `network*` allow.
      const online = sandboxSpecFor(config({ network: "unrestricted" }), cwd, temp, { permissions: "use_default" }) as SandboxSpec;
      expect(bwrapArgs(online, ["/bin/true"])).not.toContain("--unshare-net");

      // Under attn, the proxy is in the spec, so the namespace stays private.
      const proxied = sandboxSpecFor(config({ network: "proxy" }), cwd, temp, {
        permissions: "use_default", proxy: { host: "127.0.0.1", port: 4711, credentials: "run-token" },
      }) as SandboxSpec;
      expect(bwrapArgs(proxied, ["/bin/true"])).toContain("--unshare-net");

      // attn asked for a proxy but none is running: fails closed, still private.
      const missingProxy = sandboxSpecFor(config({ network: "proxy" }), cwd, temp, { permissions: "use_default" }) as SandboxSpec;
      expect(bwrapArgs(missingProxy, ["/bin/true"])).toContain("--unshare-net");
    } finally { cleanup(); }
  });
});

describe("seatbelt network policy text", () => {
  const specWith = (network: SandboxSpec["network"]): SandboxSpec => ({
    mode: "workspace-write", cwd: "/w", temp: "/w/tmp", writableRoots: ["/w"], denyRead: [], denyWrite: [], network,
  });

  test("off has no network rule, proxy allows only its loopback port, and unrestricted allows all network", () => {
    const { profile: off } = seatbeltInvocation(specWith({ mode: "off" }));
    expect(off).not.toContain("network");

    const { profile: proxy } = seatbeltInvocation(specWith({ mode: "proxy", proxy: { host: "127.0.0.1", port: 4711, credentials: "run-token" } }));
    expect(proxy).toContain('(allow network-outbound (remote ip "localhost:4711"))');
    expect(proxy).not.toContain("(allow network*)");

    const { profile: unrestricted } = seatbeltInvocation(specWith({ mode: "unrestricted" }));
    expect(unrestricted).toContain("(allow network*)");
  });
});

describe.skipIf(process.platform !== "linux")("bwrap enforcement", () => {
  test("a proxied spec still cannot reach the proxy port from inside the sandbox", async () => {
    const { cwd, temp, cleanup } = workspace();
    const proxy = await listener();
    try {
      const spec = sandboxSpecFor(config({ network: "proxy" }), cwd, temp, {
      ...
      expect(sandboxed(spec, `exec 3<>/dev/tcp/127.0.0.1/${proxy.port}`).code).not.toBe(0);
    } finally {
      proxy.close();
      cleanup();
    }
  ...
  test("an unrestricted spec reaches the network like bare pi", async () => {
    const { cwd, temp, cleanup } = workspace();
    const target = await listener();
    try {
      const spec = sandboxSpecFor(config({ network: "unrestricted" }), cwd, temp, { permissions: "use_default" });
      expect(sandboxed(spec, `exec 3<>/dev/tcp/127.0.0.1/${target.port}`).code).toBe(0);
    } finally {
      target.close();
      cleanup();
    }
  ...
  test("a proxy policy with no running proxy cannot reach the network either", async () => {
    const { cwd, temp, cleanup } = workspace();
    const target = await listener();
    try {
      const spec = sandboxSpecFor(config({ network: "proxy" }), cwd, temp, { permissions: "use_default" });
...
describe("commandEnvironment", () => {
  const spec = (proxy?: ProxyAddress): SandboxSpec => ({
    ...
    network: proxy ? { mode: "proxy", proxy } : { mode: "off" },
  });
