...
import { sandboxSpecFor, type ProxyAddress } from "../sandbox/index";
...
function session(overrides: Partial<ApprovalConfig>, proxy?: ProxyAddress) {
  ...
  return {
    root, policy,
    spec: sandboxSpecFor(source.config, source.cwd, source.temp, { permissions: "use_default", ...(proxy ? { proxy } : {}) }),
  };
}
...
test("the daemon's network switch decides the sandbox's network, not the security policy", () => {
  ...
  const off = session({ sandboxMode: "workspace-write", network: { ...defaultApprovalConfig.network, enabled: false } }, proxy);
  expect(off.policy.network).toBe("allow");
  expect(off.spec === "unsandboxed" ? undefined : off.spec.network.mode).toBe("off");
  const on = session({ sandboxMode: "workspace-write" }, proxy);
  expect(on.spec === "unsandboxed" ? undefined : on.spec.network.mode).toBe("proxy");
});

test("attn's network switch on with no running proxy fails closed instead of running unrestricted", () => {
  const on = session({ sandboxMode: "workspace-write" });
  expect(on.spec === "unsandboxed" ? undefined : on.spec.network.mode).toBe("off");
});
...
/** A registered session that answers its own approval cards, exercising the real user-choice-to-suite-report path.
 * Pass a proxy explicitly so the missing-proxy notice does not leak into amendment-notice assertions. */
async function registeredSession(reportExecPolicyAmendment: () => Promise<void>, proxy?: ProxyAddress) {
...
const runningProxy: ProxyAddress = { host: "127.0.0.1", port: 4711, credentials: "run-token" };

test("an amendment attn never recorded is said out loud and still runs the command", async () => {
  const it = await registeredSession(async () => { throw new Error("suite relay connection closed"); }, runningProxy);
...
test("an amendment attn recorded says nothing to the user", async () => {
  const it = await registeredSession(async () => {}, runningProxy);

  expect((await it.run("echo amended", { prefix_rule: ["echo"] })).exitCode).toBe(0);
  expect(it.notices).toEqual([]);
});

test("a session with network policy on but no running proxy is told it runs offline", async () => {
  const it = await registeredSession(async () => {});

  expect(it.notices).toEqual([
    {
      text: "attn configured network policy for this session but its proxy is not running, so sandboxed commands run offline. " +
