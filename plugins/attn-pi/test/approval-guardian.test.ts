...
// Not exported from the package root, but every real provider reaches it the same way, so this mirrors production.
...
test("a model call that never returns is stopped by the review deadline", async () => {
  ...
  // Sets the clock one millisecond short of the deadline right before the model call.
...
test("assistant replies stay complete so pi-ai can estimate the context", async () => {
  ...
  const provider: FakeProvider = {
    // Every real provider estimates context tokens the same way; this fake does too.
    streamSimple: (_model, context) => ({
      result: async () => {
