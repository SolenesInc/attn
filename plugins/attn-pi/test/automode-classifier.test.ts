...
function request(overrides: Partial<ClassifierRequest> = {}): ClassifierRequest {
  return {
    ...
    environment: { slots: { source_control: ["github.com/victorarias"] }, notes: [] },
    transcript: [
...
const oneModel: AutoModeConfig = { ...judgingConfig, models: ["opencode-go/glm-5.3"] };

function classifierWith(
  registry: ModelRegistryLike,
  config: AutoModeConfig = oneModel,
...
describe("classifier prompt", () => {
  ...
  test("carries the environment prose, the transcript, the call and the cwd", async () => {
    ...
    expect(call?.context.systemPrompt).toContain("**Source-control orgs**: github.com/victorarias");
  ...
  test("one cache key covers both passes, stable across the session's calls", async () => {
    ...
    const classifier = classifierWith(registry, judgingConfig, undefined, "session-7");
...
describe("the harm stage and the intent stage", () => {
  ...
  test("harm above the ceiling goes to a second pass on the same model, which can still clear it", async () => {
...
describe("classification failures fail closed", () => {
  ...
  test("a model spec that names no provider denies", async () => {
    const registry = new FakeRegistry([]);
    const config: AutoModeConfig = { ...judgingConfig, models: ["glm-5.3"] };
...
describe("the fallback walk", () => {
  ...
  test("the intent pass walks the same list, from the top", async () => {
    const registry = new FakeRegistry([dangerous(), ...errors(attemptsPerModel, "503 upstream"), graded(10)]);
    expect(await classifierWith(registry, twoDeep).classify(request())).toMatchObject({
      verdict: "allow",
      layer: "intent",
    });
    expect(specs(registry)).toEqual(["vendor/primary", "vendor/primary", "vendor/primary", "vendor/fallback"]);
  });

  test("an exhausted list on the intent pass blocks as an intent-stage outage", async () => {
...
describe("usage accounting", () => {
  test("every stage's usage is reported", async () => {
    ...
    await classifierWith(registry, judgingConfig, (usage) => reported.push(usage)).classify(request());
...
describe("a conversation the model will not take", () => {
  ...
  test("stays an outage when any model failed for another reason", async () => {
    const config: AutoModeConfig = { ...judgingConfig, models: ["p/a", "p/b"] };
  ...
  test("is still walked across the model list, in case a bigger window takes it", async () => {
    const config: AutoModeConfig = { ...judgingConfig, models: ["p/small", "p/large"] };
