...
import { securityInstructions } from "../security/guidance";
...
test("securityInstructions reports a missing attn proxy on any platform, and Linux escalation only when the proxy is running", () => {
  const { policy } = fixture();
  expect(securityInstructions(policy, { attnNetwork: "missing" })).toContain("its proxy is not running");
  expect(securityInstructions(policy, { platform: "darwin", attnNetwork: "missing" })).toContain("its proxy is not running");
  expect(securityInstructions(policy, { platform: "linux", attnNetwork: "proxy" })).toContain("unavailable inside the Linux sandbox");
  expect(securityInstructions(policy, { platform: "linux" })).not.toContain("unavailable inside the Linux sandbox");
  expect(securityInstructions(policy, { platform: "darwin", attnNetwork: "proxy" })).not.toContain("unavailable inside the Linux sandbox");
});
