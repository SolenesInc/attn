...
// Upstreams here are on loopback, so this defaults to allow_local_binding; local-guard tests turn it back off.
...
// Drains the event loop so "heard nothing yet" is a real signal, not a timing guess.
...
describe("the reader reads by demand", () => {
  ...
  test("a reader nobody is asking stops reading, so bytes stay in the client's socket", async () => {
    ...
    // Almost none of the sent traffic reached this process: the reader stays at most one chunk past the head.
