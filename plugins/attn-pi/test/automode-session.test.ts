...
function sessionWith(
  ...
  return { session: new AutoModeSession({ ...judgingConfig, ...overrides }, classifier), classifier };
}
...
describe("denial text contract", () => {
  ...
  test("a block an approval cannot lift points at the setup instead", () => {
    ...
    expect(denialToolResult({ ...call, clearable: false })).toContain("Do not ask the user to approve this one");
...
describe("session decisions", () => {
  ...
  test("the classifier is handed the environment prose and the turn signal", async () => {
    ...
    expect(classifier.requests[0]?.environment.slots.remote_targets).toEqual(["payments-prod"]);
