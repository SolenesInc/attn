...
#
#   pr-evidence.sh record  --instance <name> [--seconds N] [--out FILE]
...
cmd_record() {
  ...
  local app="" instance="" seconds=20 out=""
  while [[ $# -gt 0 ]]; do
    case "$1" in
  ...
  # Record the instance under verification (attn-<instance>.app owns a window
  # named after itself; bare "prod" is attn.app). ATTN_INSTANCE only exists in
  # the shell that eval'd instance-env, so it is a fallback, never the story.
  if [[ -z "$app" ]]; then
    instance="${instance:-${ATTN_INSTANCE:-}}"
    [[ -n "$instance" ]] || die "which window? pass --instance <name> (the instance you are verifying on) or --app <owner>"
    if [[ "$instance" == "prod" ]]; then app="attn"; else app="attn-$instance"; fi
  fi
