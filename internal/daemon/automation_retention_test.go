...
func claimTerminalAutomationRun(t *testing.T, s *store.Store, def *store.AutomationDefinition, requestID string, observedAt time.Time, resolvedLocationJSON string) *store.AutomationRun {
	...
	run, created, err := s.ClaimManualAutomationRun(def.ID, requestID, "", `{}`, def.Revision, `{}`, observedAt, store.AutomationRunReservation{
		RunID: "run-" + requestID, OccurrenceID: "occ-" + requestID, SeedID: "ticket-" + requestID, SessionID: "session-" + requestID, ProfileID: defaultProfileID(t, s),
	})
	if err != nil || !created {
...
func TestAutomationRetentionSweepPrunesCancelledRunsLikeFailed(t *testing.T) {
	...
	run, created, err := s.ClaimManualAutomationRun(def.ID, "cancelled-1", "", `{}`, def.Revision, `{}`, base, store.AutomationRunReservation{
		RunID: "run-cancelled-1", OccurrenceID: "occ-cancelled-1", SeedID: "ticket-cancelled-1", SessionID: "session-cancelled-1", ProfileID: defaultProfileID(t, d.store),
	})
	if err != nil || !created {
...
func TestAutomationRetentionSweepPendingRunsNeverPruned(t *testing.T) {
	...
	run, created, err := s.ClaimManualAutomationRun(def.ID, "pending-1", "", `{}`, def.Revision, `{}`, old, store.AutomationRunReservation{
		RunID: "run-pending-1", OccurrenceID: "occ-pending-1", SeedID: "ticket-pending-1", SessionID: "session-pending-1", ProfileID: defaultProfileID(t, d.store),
	})
	if err != nil || !created {
		t.Fatalf("claim created=%v err=%v", created, err)
	}
