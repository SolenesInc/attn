...
func TestIndexRootTruncatesAtInjectedCap(t *testing.T) {
	...
	files, truncated, err := (&Daemon{}).indexRoot(root, cap, nil)
	if err != nil {
...
func TestIndexRootAppliesCapAfterExtensionFilter(t *testing.T) {
	...
	files, truncated, err := (&Daemon{}).indexRoot(root, 5, []string{".MD"})
	if err != nil {
		t.Fatalf("indexRoot: %v", err)
	}
	if truncated {
...
func TestIndexRootUsesGitAndHonorsGitignore(t *testing.T) {
	...
	files, truncated, err := (&Daemon{}).indexRoot(root, maxFsIndexEntries, []string{"md"})
	if err != nil {
		t.Fatalf("indexRoot: %v", err)
	}
	if truncated {
