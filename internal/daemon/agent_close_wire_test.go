...
func TestTheChiefOfStaffClosesAnySessionButItself(t *testing.T) {
	...
	registerSessions(t, w, cli, "chief", "stranger")
...
func TestAgentCloseBySeedClosesItsTender(t *testing.T) {
	...
	cli := w.Client()
	cwd := registerDelegationCaller(t, w, cli, "orchestrator")
...
func TestAgentCloseRefusalsCloseNothing(t *testing.T) {
	w := newWorld(t)
	app, cli := w.App(), w.Client()
	registerSessions(t, w, cli, "chief", "worker", "other", "dupe-one", "dupe-two")
	if made := setChiefOfStaff(app, "chief", true); !made.Success {
		t.Fatalf("making chief the chief of staff = %+v", made)
	}
	...
	if live := queriedIDs(t, cli, ""); !slices.Equal(agentCloseSortedIDs(live), []string{"chief", "dupe-one", "dupe-two", "other", "worker"}) {
