...
func TestAutomationDeleteHappyPath(t *testing.T) {
	...
	run, created, err := s.ClaimManualAutomationRun(def.ID, "request-1", "", `{}`, def.Revision, `{}`, time.Now(), store.AutomationRunReservation{RunID: "run-1", OccurrenceID: "occ-1", SeedID: "s-seed01", SessionID: "session-1", ProfileID: defaultProfileID(t, d.store)})
	if err != nil || !created {
...
func TestAutomationDeleteThenReapplyResurrects(t *testing.T) {
	...
	run1, _, err := s.ClaimScheduledAutomationRun(def1.ID, "schedule:1", "singleton", def1.Revision, `{}`, `{"prompt":"v1"}`, now, store.AutomationRunReservation{RunID: "run-1", OccurrenceID: "occ-1", SeedID: "s-seed01", SessionID: "session-1", ProfileID: defaultProfileID(t, d.store)})
	if err != nil {
		t.Fatal(err)
	}
	...
	run2, fresh, err := s.ClaimScheduledAutomationRun(def2.ID, "schedule:2", "singleton", def2.Revision, `{}`, `{"prompt":"v1"}`, now.Add(24*time.Hour), store.AutomationRunReservation{RunID: "run-2", OccurrenceID: "occ-2", SeedID: "s-seed02", SessionID: "session-2", ProfileID: defaultProfileID(t, d.store)})
	if err != nil {
		t.Fatal(err)
	}
	if !fresh {
...
func TestAutomationDeleteRetiresReviewEdgesBindingsAndFencesProviderCursors(t *testing.T) {
	...
	origin, _, err := s.ClaimScheduledAutomationRun(def.ID, "schedule:1", "singleton", def.Revision, `{}`, `{}`, observedAt, store.AutomationRunReservation{
		RunID: "run-origin", OccurrenceID: "occ-origin", SeedID: "s-seed03", SessionID: "session-origin", ProfileID: defaultProfileID(t, d.store),
	})
	if err != nil {
		t.Fatal(err)
	}
