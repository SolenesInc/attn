...
func TestEvaluateCodexSignalsAreExactHead(t *testing.T) {
	...
	evidence := Evidence{
		State: "open", HeadSHA: "abcdef1234567890", HeadObservedAt: headAt,
...
func TestEvaluateCodexReactionNeedsCurrentCycle(t *testing.T) {
	headAt := time.Date(2026, 9, 19, 10, 0, 0, 0, time.UTC)
	evidence := Evidence{State: "open", HeadSHA: "abcdef", HeadObservedAt: headAt, CheckState: ChecksGreen}
...
func TestEvaluateCodexReactionCannotCrossHeadObservation(t *testing.T) {
	...
	evidence := Evidence{
		State: "open", HeadSHA: "new-head", HeadObservedAt: observedAt, CheckState: ChecksGreen,
		Reactions: []Reaction{{
...
func TestEvaluateLaterReactionSupersedesFindingAndUnavailable(t *testing.T) {
	observedAt := time.Date(2026, 9, 19, 10, 0, 0, 0, time.UTC)
	evidence := Evidence{
		State: "open", HeadSHA: "head", HeadObservedAt: observedAt, CheckState: ChecksGreen,
		Reviews: []Review{{
