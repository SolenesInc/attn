...
func originRecipients() []Recipient {
	...
	return []Recipient{
		{ID: "delegation", Description: "Visible delegated session; appended after launch instructions.", Events: []Event{
			...
			On("pr-checkout", "message_fragment", "Durable pull request checkout receipt for a new delegation.",
				template("delegation.pr-checkout", "content/delegation/pr-checkout.md",
					TextField("url", "Canonical pull request URL."), TextField("number", "Pull request number."), TextField("state", "Pull request state."),
					TextField("base_repository", "Base repository identity."), TextField("head_repository", "Head repository identity."),
					TextField("head_branch", "Remote head branch."), TextField("head_sha", "Resolved head commit."),
					TextField("local_branch", "Checked-out local branch."), TextField("worktree_path", "Checked-out worktree."),
					TextField("verified_head", "Verified worktree HEAD."), TextField("disposition", "How the checkout was prepared."),
					TextField("backup", "Optional backup branch receipt."))),
		}},
