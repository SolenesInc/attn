...
func TestClose_InteractiveShellExitsBeforeTERMGrace(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping real PTY spawn in short mode")
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if elapsed >= sigtermToHUPGrace/2 {
				t.Fatalf("Close() took %v, want under %v", elapsed, sigtermToHUPGrace/2)
			}
			t.Logf("interactive %s closed in %v", tt.name, elapsed)

			if info := s.info(); info.Running {
				t.Fatal("interactive shell still running after Close()")
			}
		})
	}
}

func TestClose_AgentUsesSIGTERM(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping real PTY spawn in short mode")
	}
