...
// For harnesses that do not report tool calls to attn; the ones with hooks record a
// `gh pr create` on their own, and the daemon dedups a second report anyway.
...
func (l Launch) Instructions() string {
	...
	// The crew block stays last so a member's charter is the freshest thing said.
