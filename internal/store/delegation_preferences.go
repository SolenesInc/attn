...
const delegationPreferencesRowColumns = `revision, parent, restores, config, source_session, message, created_at`
...
func scanDelegationPreferencesRow(row interface{ Scan(...any) error }) (delegationPreferencesRow, bool, error) {
	...
	err := row.Scan(&revision, &parent, &restores, &raw, &out.revision.SourceSession, &out.revision.Message, &createdAt)
...
func (s *Store) appendDelegationPreferences(
	...
	if _, err := tx.Exec(`INSERT INTO delegation_preference_revisions (`+delegationPreferencesRowColumns+`) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		cfg.Revision, parent, restores, string(raw), note.SourceSession, note.Message, now.Format(sortableTimeFormat)); err != nil {
		return DelegationPreferencesRevision{Config: cfg}, err
	}
