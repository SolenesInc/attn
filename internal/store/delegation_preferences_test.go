...
func TestDelegationPreferencesRollbackWalksBackAndRestoresAnyRevision(t *testing.T) {
	...
	for i, name := range names {
		...
		if _, err := s.SaveDelegationPreferences(cfg, DelegationPreferencesNote{Origin: "cli", Message: "rename to " + name}); err != nil {
			t.Fatal(err)
		}
	}
	...
	if err != nil || len(all) != 8 || all[7].Previous == nil || all[7].Previous.Revision != 0 || all[7].Message != "rename to First" || all[7].Origin != "cli" {
