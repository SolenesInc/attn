...
type LegacyTicketSeedNote struct {
	ID   string
	Body []byte
	Fact BusEvent
}

type LegacyTicketSeedSpec struct {
	TicketID              string
	SeedID                string
	SeedBody              []byte
	SeedFact              BusEvent
	...
	OriginalTerminalState TicketStatus
	CreatedAt             time.Time
}

type LegacyTicketSeedResult struct {
	SeedID string
	Result string
	Seqs   []int64
}

type LegacyTicketSeedLink struct {
	TicketID              string
	SeedID                string
	SourceKind            string
	EvidenceFingerprint   string
...
func (s *Store) EnsureLegacyTicketSeed(spec LegacyTicketSeedSpec) (LegacyTicketSeedResult, error) {
	if s == nil || s.db == nil {
		return LegacyTicketSeedResult{}, fmt.Errorf("store: no database")
	}
	seedTable, err := s.documentTable(spec.SeedSchema)
	if err != nil {
		return LegacyTicketSeedResult{}, err
	}
	noteTable, err := s.documentTable(spec.NoteSchema)
	if err != nil {
		return LegacyTicketSeedResult{}, err
	}
	dispatchTable, err := s.documentTable(spec.DispatchSchema)
	if err != nil {
		return LegacyTicketSeedResult{}, err
	}
	...
	if err := tx.QueryRow(`SELECT seed_id FROM legacy_ticket_seed_links WHERE ticket_id=?`, spec.TicketID).Scan(&existingSeedID); err == nil {
		return LegacyTicketSeedResult{SeedID: existingSeedID, Result: "adopted_link"}, nil
	} else if err != sql.ErrNoRows {
		return LegacyTicketSeedResult{}, err
	}

	lineage, err := legacyTicketSeedLineage(tx, seedTable, noteTable, dispatchTable, spec)
	if err != nil {
		return LegacyTicketSeedResult{}, err
	}
	if len(lineage) > 1 {
		return LegacyTicketSeedResult{Result: "ambiguous_lineage"}, nil
	}
	if len(lineage) == 1 {
		if err := insertLegacyTicketSeedLink(tx, spec, lineage[0]); err != nil {
			return LegacyTicketSeedResult{}, err
		}
		if err := tx.Commit(); err != nil {
			return LegacyTicketSeedResult{}, err
		}
		return LegacyTicketSeedResult{SeedID: lineage[0], Result: "adopted_lineage"}, nil
	}

	exact, err := exactLegacySeedContentMatches(tx, seedTable, spec.SeedTitle, spec.SeedDescription)
	if err != nil {
		return LegacyTicketSeedResult{}, err
	}
	if len(exact) > 0 {
		return LegacyTicketSeedResult{Result: "ambiguous_content"}, nil
	}

	expected := int64(docstore.ExpectAbsent)
	if _, err := putDocumentWith(tx, spec.SeedSchema, seedTable, spec.SeedID, spec.SeedBody, spec.CreatedAt, &expected); err != nil {
		return LegacyTicketSeedResult{}, err
	}
	seqs := make([]int64, 0, len(spec.Notes)+1)
	seq, err := appendBusEventWith(tx, spec.SeedFact, spec.CreatedAt)
	if err != nil {
		return LegacyTicketSeedResult{}, fmt.Errorf("announce recovered seed %s: %w", spec.SeedID, err)
	}
	seqs = append(seqs, seq)
	for _, note := range spec.Notes {
		if _, err := putDocumentWith(tx, spec.NoteSchema, noteTable, note.ID, note.Body, spec.CreatedAt, &expected); err != nil {
			return LegacyTicketSeedResult{}, err
		}
		seq, err := appendBusEventWith(tx, note.Fact, spec.CreatedAt)
		if err != nil {
			return LegacyTicketSeedResult{}, fmt.Errorf("announce recovered note %s: %w", note.ID, err)
		}
		seqs = append(seqs, seq)
	}
	if err := insertLegacyTicketSeedLink(tx, spec, spec.SeedID); err != nil {
		return LegacyTicketSeedResult{}, err
	}
	if err := tx.Commit(); err != nil {
		return LegacyTicketSeedResult{}, err
	}
	return LegacyTicketSeedResult{SeedID: spec.SeedID, Result: "created", Seqs: seqs}, nil
}
...
func (s *Store) LegacyTicketSeedLink(ticketID string) (*LegacyTicketSeedLink, error) {
	if s == nil || s.db == nil {
		return nil, fmt.Errorf("store: no database")
	}
	var link LegacyTicketSeedLink
	...
	err := s.db.QueryRow(`SELECT ticket_id,seed_id,source_kind,evidence_fingerprint,original_terminal_state,created_at
		...
		&link.TicketID, &link.SeedID, &link.SourceKind, &link.EvidenceFingerprint, &status, &createdAt,
	)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	...
	link.OriginalTerminalState = TicketStatus(status)
	link.CreatedAt = parseStoreTime(createdAt)
	return &link, nil
}

func insertLegacyTicketSeedLink(tx *sql.Tx, spec LegacyTicketSeedSpec, seedID string) error {
	_, err := tx.Exec(`INSERT INTO legacy_ticket_seed_links
		...
		VALUES (?,?,?,?,?,?)`, spec.TicketID, seedID, spec.SourceKind, spec.EvidenceFingerprint,
		string(spec.OriginalTerminalState), spec.CreatedAt.UTC().Format(sortableTimeFormat))
	return err
}

func legacyTicketSeedLineage(tx *sql.Tx, seedTable, noteTable, dispatchTable string, spec LegacyTicketSeedSpec) ([]string, error) {
	candidates := make(map[string]struct{})
	rows, err := tx.Query(`SELECT body FROM ` + noteTable)
	if err != nil {
		return nil, err
	}
	for rows.Next() {
		...
		if json.Unmarshal([]byte(raw), &note) != nil || note.Seed == "" {
			continue
		}
		cutover := fmt.Sprintf("converted from backlog ticket `%s` at the garden cutover", spec.TicketID)
		stranded := fmt.Sprintf("replanted from ticket `%s`,", spec.TicketID)
		if strings.Contains(note.Body, cutover) || strings.Contains(note.Body, stranded) {
			candidates[note.Seed] = struct{}{}
		}
	}
	...
	if len(sessions) > 0 {
		...
		for rows.Next() {
			...
			if json.Unmarshal([]byte(raw), &dispatch) != nil || dispatch.Crown == "" {
				continue
			}
			if _, ok := sessions[dispatch.SessionID]; ok {
				candidates[dispatch.Crown] = struct{}{}
			}
		}
		if err := rows.Close(); err != nil {
			return nil, err
	...
	for id := range candidates {
		var exists int
		if err := tx.QueryRow(`SELECT 1 FROM `+seedTable+` WHERE id=?`, id).Scan(&exists); err == nil {
			out = append(out, id)
		} else if err != sql.ErrNoRows {
			return nil, err
		}
...
func exactLegacySeedContentMatches(tx *sql.Tx, seedTable, title, body string) ([]string, error) {
