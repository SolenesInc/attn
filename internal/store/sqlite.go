...
CREATE TABLE IF NOT EXISTS app_reconcile_progress (
	...
	{150, "create setups, desktops and their panes beside the workspace tables", `
...
func migrateDB(db *sql.DB, dbPath string) error {
	...
	for _, m := range migrations {
		...
		if m.version == 141 {
			...
			if err := applyMigration150(tx, m.sql); err != nil {
				tx.Rollback()
				return fmt.Errorf("migration %d (%s): %w", m.version, m.desc, err)
			}
...
func applyMigration136(tx *sql.Tx, schema string) error {
	columns := []struct {
		name string
		sql  string
	}{
...
func applyMigration150(tx *sql.Tx, migrationSQL string) error {
